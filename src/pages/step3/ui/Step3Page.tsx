...
export function Step3Page() {
  ...
  const buildGenerateParams = useCallback(
    (section: (typeof generatedSections)[number], index: number, step3Options: Record<string, unknown> | null = null) => ({
      section,
      index,
      ...
      geminiApiKey,
      selectedAspectRatio: sectionAspectRatios[index] || selectedAspectRatio,
    ...
    [
      ...
      selectedAspectRatio, sectionAspectRatios, productName, category, productFeatures,
      additionalNotes, generatedSections, refStrength, targetAudience, designBrief,
      imageAnalysis, sectionDirectives, sectionRefFolders,
    ],
  );
  ...
  const handleRegenerate = useCallback(
    async (index: number, { withOptions }: { withOptions: boolean }) => {
      ...
      try {
        const params = buildGenerateParams(section, index, step3Options);
        ...
        console.error('[Step3] 섹션 이미지 재생성 실패:', err);
        const message = err instanceof Error ? err.message : '알 수 없는 오류';
        setGeneratedImages((prev) => ({
          ...prev,
          [index]: { data: '', prompt: '', error: message },
        }));
    ...
    [
      ...
      step3IncludeOptions, step3References, step3Prompts,
    ...
    try {
      for (let i = 0; i < total; i++) {
        ...
        if (!section) {
          ...
          setGeneratedImages((prev) => ({
            ...prev,
            [i]: { data: '', prompt: '', error: '섹션을 찾을 수 없습니다.' },
          }));
          errors++;
        ...
        try {
          const params = buildGenerateParams(section, i);
          const imageData = await generateSectionImage(params);
      ...
      console.error('[Step3] 전체 이미지 재생성 실패:', error);
