  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      ...
      const conditions = type === 'headline'
        ...
        : type === 'subcopy'
          ? '- 40자 이내의 보조 설명\n- 각각 다른 스타일과 톤으로 다양하게 작성\n- 구매 욕구를 자극하는 문구\n- 한국어로 작성'
          : '- 이미지 생성 AI(Gemini Imagen)에게 전달되는 상세한 시각적 지시문\n- 레이아웃, 조명, 색감, 구도, 분위기, 제품 배치 방식을 구체적으로 포함\n- 각각 완전히 다른 구도·조명·분위기·연출 방식으로 다양하게 작성\n- 50~150자 내외\n- 한국어로 작성';

      const currentVisualPrompt = type === 'visual' ? `\n- 현재 비주얼 지시: ${section.visualPrompt}` : '';
...
## 섹션 정보
- 섹션명: ${section.name}
- 섹션 목적: ${section.purpose}${currentVisualPrompt}
...
["첫번째", "두번째", "세번째", "네번째", "다섯번째", "여섯번째", "일곱번째", "여덟번째", "아홉번째", "열번째"]`;
      ...
      if (!text) throw new Error('AI 응답을 가져올 수 없습니다.');

      // 마크다운 코드블록 제거 후 JSON 파싱
      const cleaned = text.replace(/```(?:json)?\s*/g, '').replace(/```/g, '');
      const jsonMatch = cleaned.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        console.error('[CopySuggestModal] 카피 재생성 응답 파싱 실패:', text);
      ...
      showToast(type === 'visual' ? '새로운 비주얼 지시가 생성되었습니다!' : '새로운 카피가 생성되었습니다!', 'success');
    } catch (err) {
      console.error('[CopySuggestModal] 카피 재생성 실패:', err);
      ...
      <div className="max-h-[400px] overflow-y-auto -mx-6 px-3">
        {loading ? (
          <div className="py-10 text-center text-text-tertiary">
            <p className="mb-2">AI가 새로운 {type === 'visual' ? '비주얼 지시' : '카피'}를 생성하고 있습니다...</p>
