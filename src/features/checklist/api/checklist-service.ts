...
{"hasProductImage": true, "hasPackageImage": false, ...}`,
    ...
    if (useBackend) analysisBody.model = 'gemini-3-flash-preview';
    ...
    try {
      ...
      if (!response.ok) {
        console.error('[ChecklistService] 이미지 분석 응답 실패');
        return { analyzed: false };
      }
      ...
      console.error('[ChecklistService] 이미지 분석 API 호출 실패:', apiError);
      return { analyzed: false };
    }
  } catch (error) {
    console.error('[ChecklistService] 이미지 분석 처리 오류:', error);
    return { analyzed: false };
  }
}
...
export function generateChecklistResult(data: AnalysisData): ChecklistResult {
  ...
  // 공통 체크리스트
  for (const [, category] of Object.entries(COMMON_CHECKLIST)) {
