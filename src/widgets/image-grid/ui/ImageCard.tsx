    ...
    <div className="bg-bg-secondary border border-border-subtle rounded-[16px] overflow-hidden">
      ...
      <div className="p-4 min-h-[200px] flex items-center justify-center bg-bg-tertiary">
        {hasImage ? (
          <div
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowFullscreen(true); } }}
          ...
          <div className="text-center text-text-tertiary">
            ...
            <p className="text-sm">{img?.error || '이미지 생성 실패'}</p>
          </div>
        ) : (
          <div className="w-full aspect-[3/4] bg-bg-tertiary rounded-lg overflow-hidden relative">
            <div
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-text-quaternary text-sm">이미지 생성 대기 중</span>
            </div>
          </div>
        )}
      </div>
      ...
      {showFullscreen && img && (
        ...
        >
          <button
            onClick={() => setShowFullscreen(false)}
            className="absolute top-4 right-4 text-white text-2xl bg-white/10 rounded-full w-10 h-10 flex items-center justify-center cursor-pointer border-none hover:bg-white/20 transition-colors"
          >
            &times;
          </button>
          <img
